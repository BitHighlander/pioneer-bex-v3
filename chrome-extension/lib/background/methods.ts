...
export const handleEthereumRequest = async (method: string, params: any[], provider: JsonRpcProvider, KEEPKEY_SDK: any, ADDRESS: string): Promise<any> => {
    ...
    try {
        switch (method) {
            ...
            case 'eth_getBlockByNumber':
                console.log(tag, 'Returning eth_getBlockByNumber:', { baseFeePerGas: '0x1' });
            ...
            case 'eth_signTypedData':
                ...
                return await signTypedData(params[1], KEEPKEY_SDK);
            default:
...
const signTypedData = async (params: any, KEEPKEY_SDK: any) => {
    const tag = ' | signTypedData | ';
    try {
        console.log(tag, '**** params: ', params);
        const signedMessage = await KEEPKEY_SDK.keepkeySdk.eth.ethSignTypedData({
